...
; obColType is $40-$7F (powerups)
		...
		bhs.s	.invincible	; if yes, branch
...
.lessthan16:
		bsr.w	AddPoints
		_move.l	#ExplosionItem,obID(a1) ; change object to explosion
		clr.b	obRoutine(a1)
		tst.w	obVelY(a0)
		bmi.s	.bouncedown
		move.w	obY(a0),d0
...
HurtSonic:
		...
		jsr	(FindFreeObj).l
		bne.s	.hasshield
		_move.l	#RingLoss,obID(a1) ; load bouncing multi rings object
...
.isleft:
		...
		cmpi.l	#Spik_Solid,obID(a2)	; was damage caused by spikes?
		beq.s	.sound
...
KillSonic:
	if debugbuild
		...
		cmpi.l	#Spik_Solid,obID(a2)	; check if you were killed by spikes
		beq.s	.sound
