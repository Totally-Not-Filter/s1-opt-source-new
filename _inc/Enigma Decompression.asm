...
.nonewbyte:
		moveq	#$F,d2		; get repeat count
		and.w	d1,d2
		sub.w	d2,d1
		jmp	EniDec_00(pc,d1.w)
...
.notnewbyte:
		move.l	a0,d0
		andi.w	#$FFFE,d0
		movea.l	d0,a0		; ensure we're on an even byte
		rts
...
EniDec_FetchInlineValue:
		...
		bpl.s	.skippriority	; if not, branch
		subq.w	#1,d6
...
.skippriority:
		...
		bpl.s	.skiphighpal	; if not, branch
		subq.w	#1,d6
		btst	d6,d5
		beq.s	.skiphighpal
...
.skiphighpal:
		...
		bpl.s	.skiplowpal	; if not, branch
		subq.w	#1,d6
		btst	d6,d5
		beq.s	.skiplowpal
...
.skipxflip:
		...
		move.b	(a0)+,-(sp)
		move.w	(sp)+,d1
		move.b	(a0)+,d1	; get next word
		move.w	d1,d5		; calculate bit deficit
		lsr.l	d6,d1		; and make space for that many bits
