...
GameProgram:
		...
		btst	#6,z80_expansion_control+1
		beq.s	CheckSumCheck
...
.notPAL:
		...
		waitZ80
		moveq	#$40,d0
		move.b	d0,z80_port_1_control+1	; init port 1 (joypad 1)
...
VBlank:
		...
		move.l	v_scrposy_vdp.w,vdp_data_port ; send screen y-axis pos. to VSRAM
...
ReadJoypads:
		...
		lea	z80_port_1_data+1,a1	; first joypad port
...
.mainloop:
		move.w	#VBla_12,v_vbla_routine.w
		bsr.w	WaitForVBla
...
Tit_LoadText:
	...
	if debugbuild
		...
		copyTilemap	v_128x128,vram_fg+$208,34,22

		lea	(KosPM_GHZ_1st).l,a1 ; load GHZ patterns
...
SS_FinLoop:
		move.w	#VBla_16,v_vbla_routine.w
		bsr.w	WaitForVBla
		bsr.w	MoveSonicInDemo
		move.w	(v_jpadhold1).w,(v_jpadhold2).w
		jsr	(ExecuteObjects).w
...
loc_47D4:
		...
		move.l	#SSResult,(v_ssrescard).w ; load results screen object
...
loc_491C:
		...
		copyTilemap_End	v_ssbuffer1,ArtTile_SS_Plane_5*tile_size+plane_size_64x32,64,64
...
loc_4C10:
		...
		swap	d0
		lea	byte_4CCC(pc),a1
		lea	(KosPlus_decomp_buffer).w,a3
		moveq	#$A-1,d3
...
GM_Continue:
		...
		move.w	#tiles_to_bytes(ArtTile_Title_Card),d2
		bsr.w	Queue_KosPlus_Module
		lea	(KosPM_ContSonic).l,a1 ; load Sonic patterns
		move.w	#tiles_to_bytes(ArtTile_Continue_Sonic),d2
		bsr.w	Queue_KosPlus_Module
		lea	(KosPM_MiniSonic).l,a1 ; load continue screen patterns
		move.w	#tiles_to_bytes(ArtTile_Mini_Sonic),d2
		bsr.w	Queue_KosPlus_Module
		moveq	#10,d1
		...
		move.l	#ContSonic,(v_player).w ; load Sonic object
		move.l	#ContScrItem,(v_continuetext).w ; load continue screen objects
		move.l	#ContScrItem,(v_continuelight).w
		move.w	#3*$80,(v_continuelight+obPriority).w
		move.b	#4,(v_continuelight+obFrame).w
		move.l	#ContScrItem,(v_continueicon).w
...
GM_Credits:
		...
		lea	(KosPM_CreditText).l,a1 ; load credits alphabet patterns
		move.w	#tiles_to_bytes(ArtTile_Credits_Font),d2
		bsr.w	Queue_KosPlus_Module
		...
		moveq	#palid_Sonic,d0
		bsr.w	PalLoad_Fade	; load Sonic's palette
		move.l	#CreditsText,(v_credits).w ; load credits object
		jsr	(ExecuteObjects).w
		jsr	(BuildSprites).w
...
Cred_WaitLoop:
		...
		tst.w	(KosPlus_modules_left).w ; have level gfx finished decompressing?
		bne.s	Cred_WaitLoop	; if not, branch
...
EndingDemoLoad:
		...
		lea	(v_lastlamp).w,a2
		moveq	#(EndDemo_LampVar_End-EndDemo_LampVar)/4-1,d0
...
TryAgainEnd:
		...
		bsr.w	AddPLC	; load "TRY AGAIN" or "END" patterns
		...
		move.l	#EndEggman,(v_endeggman).w ; load Eggman object
		jsr	(ExecuteObjects).w
		jsr	(BuildSprites).w
...
Blk16_GHZ:	binclude	"map16/GHZ.eni"
		even
KosPM_GHZ_1st:	binclude	"artkospm/8x8 - GHZ1.kospm"	; GHZ primary patterns
		even
KosPM_GHZ_2nd:	binclude	"artkospm/8x8 - GHZ2.kospm"	; GHZ secondary patterns
		even
...
KosPM_TryAgain:	binclude	"artkospm/Ending - Try Again.kospm"
		even
KosPlus_EndFlowers:	binclude	"artkosp/Flowers at Ending.kosp" ; ending sequence animated flowers
		even
