...
;	bne.s	locret_71CAA				; Return if yes
	...
	moveq	#signextendB($80),d1				; Enable DAC
	bsr.w	WriteFMI

	; Update DAC's panning (the panning may have been changed by the FM6 track)
	moveq	#signextendB($B6),d0				; Register for FM6/DAC AMS/FMS/Panning
	move.b	SMPS_Track.AMSFMSPan(a5),d1	; Value to send
	bsr.w	WriteFMII

.dac_already_enabled:
	SMPS_stopZ80_safe
	move.b	d2,SMPS_z80_ram+Z_MPCM_CommandInput
	SMPS_startZ80_safe
...
.cap:
	SMPS_stopZ80_safe
	move.b	#$F,MPCM_Z80_RAM+Z_MPCM_VolumeInput
	SMPS_startZ80_safe
	rts
.do_not_cap:
	lsr.b	#3,d0
...
.clearramloop:
	...
	; Reset DAC volume
	moveq	#0,d0	; Clownacy | Reset DAC volume to maximum
...
; ---------------------------------------------------------------------------
	...
	include	"sound/Sonic-2-Clone-Driver-v2/engine/MegaPCM.asm"
	include	"sound/Sonic-2-Clone-Driver-v2/engine/SampleTable.asm"